...
/// [length] is how far from the center we want the point to end up (eg: hand length)
...
/// rendering by pulling it into its owen painter
class _ClockDecorationPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Clock Numbers
    final TextStyle ts =
        new TextStyle(color: color, fontWeight: FontWeight.bold);
    ...
    for (int i = 1; i <= OctalDuration.HOURS_PER_SCISMA; i++) {
      ...
      final Offset paintOffset = _getPosition(i.toDouble(),
          OctalDuration.HOURS_PER_SCISMA.toDouble(), maxLen * 0.85);
    ...
    const underHour =
        OctalDuration.MINUTES_PER_HOUR / OctalDuration.HOURS_PER_SCISMA;
    for (int i = 0; i < OctalDuration.MINUTES_PER_HOUR; i++) {
      double insideFactor = .95;
      if (i.remainder(underHour) == 0) insideFactor = .93;

      paint.colorFilter = (i.remainder(4) == 0)
          ? new ColorFilter.mode(color, BlendMode.darken)
          : null;

      final Offset outsideOffset = _getPosition(i.toDouble(),
          OctalDuration.MINUTES_PER_HOUR.toDouble(), maxLen - 1.5);
      final Offset insideOffset = _getPosition(i.toDouble(),
          OctalDuration.MINUTES_PER_HOUR.toDouble(), maxLen * insideFactor);
...
class _ClockHandsPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Second hand
    final Offset secondsOffset = _getPosition(oct2dec(second).toDouble(),
        OctalDuration.SECONDS_PER_MINUTE.toDouble(), maxLen * .8);
    canvas.drawLine(center, center + secondsOffset, paint);

    // Minute hand
    paint.strokeWidth = 4.0;
    final int minutes = oct2dec(minute);
    final Offset minuteOffset = _getPosition(minutes.toDouble(),
        OctalDuration.MINUTES_PER_HOUR.toDouble(), maxLen * .75);
    canvas.drawLine(center, center + minuteOffset, paint);

    // Hour hand
    paint.strokeWidth = 6.0;
    final int hourMinutes =
        oct2dec(hour) * OctalDuration.MINUTES_PER_HOUR + minutes;
    final int scismaMinutes =
        OctalDuration.HOURS_PER_SCISMA * OctalDuration.MINUTES_PER_HOUR;
    final Offset hourOffset = _getPosition(
        hourMinutes.toDouble(), scismaMinutes.toDouble(), maxLen * .5);
